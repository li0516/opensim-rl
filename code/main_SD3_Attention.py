...
import models_Attention as models
...
if __name__ == "__main__":
    ...
    actor_1 = models.Actor(state_dimension, action_dimension, action_max, ram=ram)
    target_actor_1 = models.Actor(state_dimension, action_dimension, action_max, ram=ram)
    actor_optimizer_1 = torch.optim.Adam(actor_1.parameters(), lr=0.00001)

    actor_2 = models.Actor(state_dimension, action_dimension, action_max, ram=ram)
    target_actor_2 = models.Actor(state_dimension, action_dimension, action_max, ram=ram)
    ...
    for t in range(1000000):
        ...
        if t < 100:
            ...
            action_with_parameter_ou_noise = np.clip(
                action_with_parameter_noise + np.random.normal(0, 0.2, size=action_dimension), 0, 1)
        else:
            # initial_state = Variable(torch.from_numpy(state)).reshape(1, -1)

            data_list = list(ram.buffer)
            data_list = np.array(data_list[-10:])
            actor_copy_state, actor_copy_action, _, _, _ = zip(*data_list)
            actor_copy_actions = np.array(actor_copy_action)
            actor_copy_states = np.array(actor_copy_state)
            q = []
            for a_ in actor_copy_actions:
                # s = Variable(torch.from_numpy(s_))
                # s = Variable(state)
                a_ = torch.from_numpy(a_).float()
                # action = Actor(273, 22, 1).forward(s__)
                q.append(float(critic_1.forward(initial_state, a_)))
            initial_state = np.dot(np.array(F.softmax(torch.Tensor(q), -1)), actor_copy_states)
            initial_state = Variable(torch.from_numpy(initial_state)).reshape(1, -1)
