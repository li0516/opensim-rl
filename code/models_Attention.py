...
class Actor(nn.Module):
    def __init__(self, state_dimension, action_dimension, action_max, ram=None):
    ...
    def forward(self, state):
        ...
        self.ln1 = nn.LayerNorm(output.shape[-1], elementwise_affine=False, eps=1e-4)
        ln1_out = self.ln1(output)
        output = F.relu(ln1_out)

        output = self.fc2(output)
        self.ln2 = nn.LayerNorm(output.shape[-1], elementwise_affine=False, eps=1e-4)
        ln2_out = self.ln2(output)
        output = F.relu(ln2_out)

        output = self.fc3(output)
        self.ln3 = nn.LayerNorm(output.shape[-1], elementwise_affine=False, eps=1e-4)
